...
export default function SuperAdminPage() {
  ...
  const [includeStarterProducts, setIncludeStarterProducts] = useState(true);
    ...
    if (data) {
      ...
      if (shopIds.length) {
        try {
          ...
          console.warn(
            "shop_payment_info table not found yet, skipping QR data load",
          );
          setShopQrById({});
        }
      } else {
  ...
  const togglePause = async (shop: Shop) => {
    try {
      ...
      toast({
        title: "Success",
        description: shop.isPaused ? "Shop unpaused!" : "Shop paused!",
      });
      loadShops();
    } catch (e) {
      console.error("Unexpected error:", e);
  ...
  const deleteShop = async (shop: Shop) => {
    ...
    try {
      ...
      if (userDeleteError) {
        ...
        toast({
          title: "Error",
          description: `Failed to delete users: ${userDeleteError.message}`,
        });
        return;
      }

      const { error: shopDeleteError } = await (supabase as any)
        .from("shops")
        .delete()
        .eq("id", shop.id);
      ...
      if (shopDeleteError) {
        ...
        toast({
          title: "Error",
          description: `Failed to delete shop: ${shopDeleteError.message}`,
        });
        return;
      }
  ...
  const resetForm = () => {
    ...
    setIncludeStarterProducts(true);
    setEditingShop(null);
  };
      ...
      try {
        ...
        if (data) {
          if (typeof data.qr_code_url === "string")
            setBillingQrImageUrl(data.qr_code_url);
        }
      } catch (error) {
        // Ignore errors from missing table
        console.warn(
          "shop_payment_info table not found yet, skipping existing data load",
        );
      }
    })();
  };
      ...
      try {
        ...
        if (fetchError) {
          ...
          toast({
            ...
            description:
              "Please apply the latest database schema to your Supabase project first. See the instructions in the schema.sql file.",
          });
          return;
        }
        ...
        if (formattedError.includes("Could not find the table")) {
          ...
          toast({
            title: "Error",
            description:
              "An error occurred while saving. Please check your database connection.",
          });
        }
      }
    } catch (e: any) {
  ...
  const markPaidAndExtend = async () => {
    ...
    const now = new Date();
    const currentEnd = billingShop.subscriptionEndDate
      ? new Date(billingShop.subscriptionEndDate)
      : null;
    const base =
    ...
    try {
      ...
      try {
        ...
        if (subError) {
          ...
          console.warn(
            "Could not create subscription record (table might not exist yet):",
            formattedError,
          );
        ...
        console.warn(
          "Error creating subscription record (table might not exist yet)",
        );
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold text-green-600">
              {shops.filter((s) => !s.isPaused).length}
            </p>
          </CardContent>
        </Card>
        <Card>
            ...
            <CardContent className="p-6">
              <div className="flex flex-col lg:flex-row items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  <div
                    className={`p-3 rounded-full ${shop.isPaused ? "bg-orange-100" : "bg-green-100"}`}
                  >
                    <Store
                      className={`h-6 w-6 ${shop.isPaused ? "text-orange-600" : "text-green-600"}`}
                    />
                  </div>
                  <div>
                ...
                <div className="flex flex-wrap gap-2 w-full lg:w-auto">
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => openBillingDialog(shop)}
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-4">
            ...
            {!editingShop && (
              <div className="flex items-center justify-between gap-4 rounded-lg border bg-muted/30 p-3">
                <div>
                  <p className="text-sm font-medium">Starter products</p>
                  <p className="text-xs text-muted-foreground">
                    Include the ready-to-use item list at quantity 0 for this
                    shop only.
                  </p>
                </div>
                <div className="flex shrink-0 gap-2">
                  <Button
                    type="button"
                    size="sm"
                    variant={includeStarterProducts ? "default" : "outline"}
                    onClick={() => setIncludeStarterProducts(true)}
                  >
                    On
                  </Button>
                  <Button
                    ...
                    size="sm"
                    variant={!includeStarterProducts ? "default" : "outline"}
                    onClick={() => setIncludeStarterProducts(false)}
                  >
                    Off
                  </Button>
                </div>
          ...
          <DialogFooter className="flex flex-col sm:flex-row gap-2">
            ...
            <Button
              ...
              className="w-full sm:w-auto"
            >
              {isCreatingShop ? "Saving..." : editingShop ? "Update" : "Create"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
            ...
            <div className="space-y-4 py-2">
              <div className="text-sm p-3 bg-gray-50 rounded-lg">
                <div className="font-semibold text-base">
                  {billingShop.shopName}
                </div>
                <div className="text-gray-500">
                  Owner: {billingShop.ownerName}
                </div>
              </div>
