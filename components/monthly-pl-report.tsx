...
export function MonthlyPLReport() {
  ...
  const { monthSales, dailyData, totalRevenue, totalCost, totalProfit, profitMargin } = useMemo(() => {
    setLoading(true);
    
    try {
      const monthSalesData = sales.filter(s => s.date.startsWith(selectedMonth));
      ...
      const totalRevenueCalc = monthSalesData.reduce((sum, s) => sum + s.subtotal, 0);
      const totalCostCalc = monthSalesData.reduce((sum, s) => sum + s.totalCost, 0);
      const totalProfitCalc = totalRevenueCalc - totalCostCalc;
      const profitMarginCalc = totalRevenueCalc > 0 ? ((totalProfitCalc / totalRevenueCalc) * 100).toFixed(1) : 0;
  ...
  return (
    <div className="space-y-6 pb-10">
      <div>
        ...
        <p className="text-muted-foreground mt-2">Profit and Loss analysis for your shop</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-muted-foreground">No sales data for this month</div>
      ) : (
        <>
      {/* Summary Cards */}
      <div className="grid grid-cols-2 gap-3 sm:gap-4">
        ...
        <Card className="border-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold">₹{totalCost.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</div>
          </CardContent>
        </Card>
      ...
      {monthSales.length === 0 && !loading && (
        <Card className="border-2 border-dashed">
          <CardContent className="pt-8 pb-8 text-center">
            <p className="text-muted-foreground">No sales data for this month</p>
          </CardContent>
        </Card>
      )}
        </>
